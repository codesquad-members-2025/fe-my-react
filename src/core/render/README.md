...
- 생성된 DOM 노드에 `addEventListener`를 사용하여 이벤트 리스너를 등록한다. -> 해당 render 함수에서는 이벤트 위임 방식으로 이밴트 핸들링합니다.
...
#### 4. 엔드포인트에 렌더링된 vittual-DOM을 자식으로 삽입한다.

- 최종적으로는 위의 과정들로인해 가상DOM이 생성된다.

  - 왜 가상 돔일까?
    - 아직 실제 DOM에 삽입하지 않고 JS객체로 다루기 떄문에 DOM이라고 부를 수 없다.
    - 리액트에서는 DOM이 아닌 virtual-DOM 상태에서 재조정과정을 실행한다. 가상돔은 JS객체이기에 비용이 더 저렴하고 속도도 바르기 떄문에 성능을 높일 수 있다.
    - 그래서 반드시 render함수 안에서도 실제 DOM으로 변환하는 과정은 virtual-DOM이 모두 준비가 되었을떄 최종적으로 실행해야한다.

- 실제 DOM의 자식으로 생성된 virtual-DOM 을 삽입해야 한다.
- render함수는 만들어진 가상 DOM이 어디에 들어가야할지 알아야한다.
